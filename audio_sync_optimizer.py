...
def interpolate_missing_timestamps(target_tokens: list, actual_timestamps: list) -> list:
    ...
    Strategi: Linear interpolation dengan weighting berdasarkan posisi.
    """
    ...
    for target_idx, target in enumerate(target_tokens):
        if actual_idx < len(actual_timestamps):
            ...
            # Interpolasi dari kata terakhir
            if result:
                last_ts = result[-1]
                interpolated = {
                    ...
                    "start": last_ts["end"],
                    "end": last_ts["end"] + 0.25,  # Default 250ms
                    "duration": 0.25,
                    "section": target.get("section", "body"),
                    "confidence": 0.40  # Low confidence untuk interpolated
                }
                result.append(interpolated)
            else:
                # First word belum match (edge case)
                result.append({
                    "word": target.get("word", ""),
                    "display": target.get("token", ""),
                    "start": 0.0,
...
def smooth_duration_outliers(timestamps: list, std_dev_threshold: float = 2.0) -> list:
    ...
    for ts in timestamps:
        ...
        # Deteksi outlier
        if abs(duration - median_duration) > std_dev_threshold * std_duration:
            ts_copy = ts.copy()
            # Gunakan moving average dari tetangga
            ts_copy["duration"] = median_duration
            ts_copy["end"] = ts_copy["start"] + median_duration
...
async def optimize_subtitle_timing(
    ...
    # Step 1: Voice Activity Detection untuk gap detection
    if enable_vad:
        try:
    ...
    # Step 3: Interpolasi untuk missing tokens
    ...
    # Step 4: Validasi continuity (tidak boleh ada gap atau overlap)
    optimized = _validate_and_fix_continuity(optimized)
    
    logger.info(f"✅ Optimasi selesai. Total timestamps: {len(optimized)}")
    return optimized

def _validate_and_fix_continuity(timestamps: list) -> list:
    """
    Pastikan tidak ada gap atau overlap antara subtitle.
    """
    if len(timestamps) <= 1:
        return timestamps
    ...
    for idx, ts in enumerate(timestamps):
        ...
        if idx > 0:
            prev_ts = result[-1]
            # Cegah overlap: jika start < prev end, geser start
