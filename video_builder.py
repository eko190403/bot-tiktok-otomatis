...
async def generate_structured_script(channel_cfg: dict) -> dict:
    ...
    prompt = (
        ...
        "ATURAN WAJIB:\n"
        f"{hook_rule}"
        "2. 'story': Penjelasan mendalam yang emosional, menggunakan angka/statistik spesifik (misal '93% orang tidak sadar'), analogi sederhana, dan membangun rasa penasaran. MINIMAL 4 kalimat, MAKSIMAL 6 kalimat. SANGAT PENTING (DRAMATIC PAUSE): Tepat sebelum kamu mengungkapkan fakta paling mengejutkan atau plot twist di cerita ini, sisipkan tanda elipsis panjang '... [JEDA] ...' untuk memaksa suara AI berhenti sejenak dan membangun ketegangan. Pastikan total kata naskah (hook + story + cta) MAKSIMAL 110 kata agar tidak terlalu padat. PERBANYAK PENGGUNAAN TANDA BACA KOMA (,) di dalam kalimat untuk memaksa AI pembaca suara memperlambat tempo (pacing) dan menciptakan jeda-jeda dramatis yang mengancam/berwibawa.\n"
        "3. 'cta': Ajakan bertindak yang personal dan mendalam, maks 2 kalimat. PENTING: Sama seperti story, PERBANYAK KOMA (,) dan sisipkan minimal satu '... [JEDA] ...' sebelum kalimat penutup CTA untuk memberi ruang nafas agar suara AI tetap lambat, berat, dan tenang di akhir video (JANGAN terburu-buru/kebut). Kalimat terakhir dari cta ini HARUS dirancang menggantung di akhir kata dan secara tata bahasa menyambung kembali dengan mulus (seamless loop) ke kata pertama pada kalimat HOOK utama agar video bisa ditonton berulang kali secara melingkar tanpa terputus. Contoh jika HOOK = 'KENAPA KAMU MISKIN', maka akhir CTA bisa berbunyi '...dan itulah alasan utama...' sehingga saat diputar ulang ia tersambung menjadi '...dan itulah alasan utama KENAPA KAMU MISKIN'.\n"
        ...
        "GAYA BAHASA: Gunakan Bahasa Indonesia percakapan yang natural, energetik, dan terasa personal seolah berbicara langsung ke satu orang. SANGAT PENTING: Hindari terjemahan kaku dari bahasa Inggris. Pastikan kalimat pembuka (hook) dan sambungan looping-nya 100% masuk akal secara gramatikal (contoh SALAH: 'Apa yang kamu takutkan mereka tak perhatikan', contoh BENAR: 'Apa yang kamu takutkan, sebenarnya tak mereka perhatikan').\n"
...
async def create_video(channel_id: str = "ruangpikir") -> bool:
    ...
    try:
        ...
        def execute_ffmpeg_render(target_path: str):
            ...
            # CRF 23 (Kualitas Tinggi) + maxrate 4000k (Safety Limit 50MB absolut)
            params = ["-crf", "23", "-maxrate", "4000k", "-bufsize", "8000k", "-pix_fmt", "yuv420p", "-g", "15"]
            if bg_type == "pexels":
