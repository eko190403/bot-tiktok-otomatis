...
async def generate_structured_script(channel_cfg: dict) -> dict:
    ...
    if bg_type == "hunter":
        ...
        prompt = (
            ...
            f"{ab_test_instruction}\n"
            "GAYA BAHASA: Gunakan gaya bahasa Gen-Z yang 'Rebel', blak-blakan, dan agak 'toxic' (roasting penonton secara halus). Gunakan slang tongkrongan (lu, gue, anjir, gila, bangsatnya, sumpah). JANGAN pakai bahasa baku, sastra kuno, atau sok puitis. Jadilah seperti teman yang sedang memarahi atau menyindir kelakuan bodoh penonton. Pastikan kalimat pembuka (hook) dan sambungan looping-nya tetap nyambung secara gramatikal meski bahasanya gaul.\n"
            f"OUTPUT: Hanya JSON murni dengan key 'hook', 'story', 'cta', 'caption', 'tags', 'category_id', 'interactive_comment', 'yt_title', dan 'yt_description'. Jika diinstruksikan A/B test, sertakan key 'hook_b'. Tidak ada teks lain di luar JSON.{exclude_prompt}{performance_prompt}{comment_insight_prompt}{trends_prompt}"
        )
